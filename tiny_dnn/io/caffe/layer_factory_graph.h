...
const caffe::NetParameter &layer, const shape3d &data_shape) {
  ...
  shape_t *shapes_next;
  shapes_next = (shape_t*)malloc(sizeof(shape_t) * src_net.size());
  shapes_next[0] = shape;

  std::map<std::string, int> layer_name_index;

  for (size_t i = 0; i < src_net.size(); i++) {
    layer_name_index[src_net[i].name()] = i;
    ...
    if (src_net[i].bottom_size() == 0) {
      ...
      *dst_net << layer;
      ...
      shapes_next[i] = shape_next;
    }
    else if (src_net[i].bottom_size() == 1) {
      //single input
      auto bottom_local = src_net[i].bottom(0);
      shape = shapes_next[layer_name_index[bottom_local]];
      auto layer         = detail::create(src_net[i], shape, &shape_next);
      nn_info("convert " + type + " => " + typeid(*layer).name());
      nn_info("shape:" + to_string(shape_next));
      ...
      *dst_net << layer;
      ...
      //multiple inputs
      std::vector<shape3d> shapes_local (src_net[i].bottom_size());
      for (int j = 0; j < src_net[i].bottom_size(); ++j)
      {
        auto bottom_local = src_net[i].bottom(j);
        shapes_local[j] = shapes_next[layer_name_index[bottom_local]];
      }

      auto layer         = detail::create_multiple(src_net[i], shapes_local, &shape_next);
      nn_info("convert " + type + " => " + typeid(*layer).name());
      nn_info("shape:" + to_string(shape_next));
