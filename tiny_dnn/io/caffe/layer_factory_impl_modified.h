...
inline std::shared_ptr<layer> create(const caffe::LayerParameter &layer,
  ...
  //return detail::create_concat(layer, in_shape, out_shape);
  //}

  throw nn_error("layer parser not found");
}
inline std::shared_ptr<layer> create_concat(const caffe::LayerParameter &layer,
    const std::vector<shape3d> in_shapes,
    shape_t *out_shape) {
  auto power = std::make_shared<concat_layer>(in_shapes);
  *out_shape = power->out_shape()[0];
  return power;
}

inline std::shared_ptr<layer> create_multiple(const caffe::LayerParameter &layer,
    const std::vector<shape3d> in_shape,
    shape_t *out_shape) {
  const std::string layer_type = layer.type();

  if (layer_type == "Concat") {
    return detail::create_concat(layer, in_shape, out_shape);
    //throw nn_error("found concat but layer parser not found");
  }
